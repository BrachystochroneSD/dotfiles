...
(use-package mu4e
  ...
  (defun my-mu4e-delete-from ()
    (interactive)
    (let* ((from (car (mu4e-field-at-point :from)))
           (from-email (plist-get from :email))
           (from-name (plist-get from :name))
           (from-regex (or from-name from-email)))
      (my-mu4e-mark-as-delete-regexp (if (length> from-regex 21)
                                         (substring  from-regex 0 22)
                                       from-regex))))

  (defun my-mu4e-refile-from ()
    (interactive)
    (my-mu4e-mark-as-refile-regexp (plist-get (car (mu4e-field-at-point :from)) :name)))
